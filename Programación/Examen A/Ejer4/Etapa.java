...
public class Etapa {
    ...
    // mierda vieja que uso para guiarme
    ...
    public void clasificacionEquipos() {
        Collections.sort(listaEquipos); // ordenarlo

        int posicion = 1; // para la posicion

        for (Equipo equipo : listaEquipos) {
            System.out.println(posicion + " " + equipo.getNombre() + " Tiempo total: " + equipo.getTotalTiempo());
            posicion++;
        }
    }
    ...
    public void calcularClasificacion() {
        for (Ciclista c : participantes) {
            double tiempoEtapa = c.getTiempoAcumulado();
            c.setTiempoAcumulado(c.getTiempoAcumulado() + tiempoEtapa);
